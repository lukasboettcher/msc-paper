...
As a result, pointer analyses have to produce an over-approximation of the targets each pointer can point to at runtime.
...
This showcases the tradeoff that all non-theoretical pointer analyses have to make in oder to be applicable to real programs and avoid undecidability.
...
For the given example, \mint{c}|p1.age| and \mint{c}|p1.name| can point to different memory locations.
Alternatively a field-insensitive analysis does not differentiate betweeb any fields of a given struct.
Therefore only two nodes are created to represent the struct, $p1.*$ and $p2.*$.
Another common alternative is field-base-sensitivity, where instead of omitting the individual fields of each struct, the fields of every struct are merged into a single instance of the struct.
As a result the Person structs, p1 and p2, would be represented as a single object with fields name and age, such that $p1.age == p2.age$ are represented by the same node in the analysis.
